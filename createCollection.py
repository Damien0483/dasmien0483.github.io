...
for line in open('dataset_en_dev.json','r'):
    ...
    productList.append([dataSet["product_id"],
...
categoryList.sort()
for category in categoryList:
    query = "MATCH (:Product)--(category:Category) WHERE category.name ='" + category + "' RETURN(COUNT(*))" result = session.run(query) print(category + " has " + str(result.value()) + "products!")

#Delete all relationships connected to 1 node
print("\nAll relationships from book category removed.")
query = "MATCH (node:Category{name:'book'})-[r:CLASIFIED_AS]->() DELETE r" session.run(query)

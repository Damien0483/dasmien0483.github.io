...
for line in open('dataset_en_dev.json', 'r'):
    ...
    insertReviewsPrep = ''' INSERT INTO Reviews (ReviewID, ReviewerID, ProductID, Category, Stars, Title, Content, Language) VALUES(%s, %s, %s, %s, %s, %s, %s, '''
    
    #Execute INSERT statement using the JSON dataset
    session.execute(insertReviewsPrep, [dataSet["review_id"], dataSet["reviewer_id"], dataSet["product_id"], dataSet["product_category"], int(dataSet["stars"]), dataSet["review_title"], dataSet["review_body"], dataSet["language"]])
    
    #Prepare INSERT statement to add in new data into Categories
    insertCategoriesPrep = '''
                INSERT INTO Categories (Category, Stars, ProductID) VALUES(%s, %s, %s); '''
    #Execute INSERT statement using the JSON dataset
    session.execute(insertCategoriesPrep, [dataSet["product_category"], int(dataSet["stars"]), dataSet["product_id"]])
...
query = '''SELECT COUNT(*) FROM Reviews WHERE Category = 'wireless' AND stars > 3 ALLOW FILTERING;'''
results = session.execute(query)
...
query = '''SELECT MIN(stars), MAX(stars) FROM Reviews WHERE productID = 'product_en_0947733' ALLOW FILTERING;'''
